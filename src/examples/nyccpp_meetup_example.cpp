...
auto make_service(pion::scheduler& scheduler) -> std::unique_ptr<dslam::service> {
    ...
    auto resolver =
        std::make_shared<pion::stdx::asio::ip::tcp::resolver>(scheduler.get_io_service());
    ...
    return std::unique_ptr<dslam::service>(new dslam::service{
        ...
        {true,
         [counter](context const& context) -> result {
             counter->record(context);
             return dslam::next_handler;
         }},

        {"hello", get or post, [](context const& context) -> result { return "Hello, World!\n"; }},
        ...
        {"chunks",
         ...
         [](context const& context) -> result {
             ...
             writer->send_chunk(
                 [context](pion::stdx::error_code const& error, std::size_t written) {
        ...
        {"resolve/:host",
         get,
         [resolver](context const& context) -> result {
             const std::string host = context->match("host");
             context->writer() << "Resolving " << host << "\n";
             resolver->async_resolve(
                 host,
                 [context](pion::stdx::error_code const& err,
...
int main(int argc, char* argv[]) {
    ...
    pion::stdx::asio::ip::tcp::endpoint const where(pion::stdx::asio::ip::tcp::v4(), port);
