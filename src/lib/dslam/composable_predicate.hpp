...
namespace dslam {
  ...
  public:
    ...
    composable_predicate(predicate_type const& predicate) :
      predicate_(predicate) {}

    // NOTE(C++11): rvalue references
    composable_predicate(predicate_type&& predicate) :
      predicate_(predicate) {}
  ...
  template<typename ...ArgTypes>
  composable_predicate<bool(ArgTypes...)>
  operator or(composable_predicate<bool(ArgTypes...)> const& l,
	      composable_predicate<bool(ArgTypes...)> const& r) {
    return composable_predicate<bool(ArgTypes...)>([l, r](ArgTypes&&... args) {
      return
	l(std::forward<ArgTypes>(args)...)
	or
	r(std::forward<ArgTypes>(args)...);
    });
  }

  template<typename ...ArgTypes>
  composable_predicate<bool(ArgTypes...)>
  operator and(composable_predicate<bool(ArgTypes...)> const& l,
	       composable_predicate<bool(ArgTypes...)> const& r) {
    return composable_predicate<bool(ArgTypes...)>([l, r](ArgTypes&&... args) {
      return
	l(std::forward<ArgTypes>(args)...)
  ...
  template<typename ...ArgTypes>
  composable_predicate<ArgTypes...>
  operator not(composable_predicate<ArgTypes...> const& p) {
    return composable_predicate<ArgTypes...>([p](ArgTypes&&... args) {
      return not p(std::forward<ArgTypes>(args)...);
    });
  }
