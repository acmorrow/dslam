...
service::service(std::initializer_list<route> routes) : routes_(routes) {}
...
service::context::context(class service& service,
    ...
    : service_{service},
      ...
      writer_{response_writer::create(connection_,
                                      *request_,
                                      std::bind(&do_finish, connection_, std::placeholders::_1))},
...
void service::context::run_in_thread(const task& task) {
    ...
    std::thread t([this, task] { this->dispatch_result(task()); });
    t.detach();
}
